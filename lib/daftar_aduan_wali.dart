...
import 'package:aplikasi_aduan/login_wali.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

class DaftarAduanWali extends StatefulWidget {
  const DaftarAduanWali({required this.email, super.key});
  final String email;
  // const DaftarAduanwali({required this.email});

  @override
  State<DaftarAduanWali> createState() => _HalamanDaftarAduanwali();
}

int currentPage = 1;
final int complaintsPerPage = 4;

class _HalamanDaftarAduanwali extends State<DaftarAduanWali> {
  ...
  try {
    final querySnapshot = await FirebaseFirestore.instance
        .collection('aduan')
        .where('email', isEqualTo: widget.email) // Menambahkan filter emailAnak
        .get();
  ...
  void logout() {
    Navigator.pushAndRemoveUntil(
      context,
      MaterialPageRoute(builder: (context) => LoginPageWali()),
    ...
    Navigator.pushNamed(
      ...
      arguments: widget.email,  // Mengirimkan email ke halaman BuatAduan
            ...
            ListTile(
              ...
              onTap: () {
                Navigator.pushNamed(context, '/daftarAduanwali');
              },
            ),
            Divider(),
            ListTile(
      ...
      body: isLoading
          ...
          context,
          MaterialPageRoute(
            builder: (context) => DetailAduanWali(judulAduan: complaint['judulAduan']),
          ),
        );
                                    },
                                    child: Text('Detail'),
